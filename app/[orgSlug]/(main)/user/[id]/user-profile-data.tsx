...
export async function UserProfileData({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fast identity + privacy checks: a single-row user lookup decides the
  // profile frame, while the class/resource queries stream below the shell.
  ...
  const profileUserRole = await getUserRole(userId)
  ...
  return (
    <UserProfileContent
      ...
      isPrivate={isPrivate}
      showClasses={profileUser.showClasses}
      showResources={profileUser.showResources}
    />
  )
}
...
async function UserProfileContent({
  ...
  const createdClasses = (profileUser.role === "teacher" && (isOwnProfile || showClasses))
    ? await db
      .select({
        id: classes.id,
        title: classes.title,
  ...
  const enrolledClassesData = (isOwnProfile || showClasses)
    ? await db
      .select({
        id: classes.id,
        title: classes.title,
  ...
  const createdResources = (isOwnProfile || showResources)
    ? await db
      .select()
      .from(resources)
